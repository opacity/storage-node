...
func (stripePayment *StripePayment) SendUpgradeOPCT(account Account, newPlanID uint, networkID uint) error {
	upgrade, _ := GetUpgradeFromAccountIDAndPlans(account.AccountID, newPlanID, account.PlanInfo.ID)
